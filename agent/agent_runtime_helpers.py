...
_STRAY_THINK_RE = re.compile(
    r'</?(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)>\s*',
    flags=re.IGNORECASE
)

_STRAY_TOOL_RE = re.compile(
...
def strip_think_blocks(agent, content: str) -> str:
    ...
    # 1c. Gemma-style standalone tool call
    content = _GEMMA_TOOL_RE.sub('', content)

    # 2. Unterminated reasoning block
    content = _UNTERMINATED_THINK_RE.sub('', content)

    # 3. Stray orphan open/close tags
    content = _STRAY_THINK_RE.sub('', content)

    # 3b. Stray tool-call closers
    content = _STRAY_TOOL_RE.sub('', content)
