...
_REASONING_RE = re.compile(r'<reasoning>.*?</reasoning>', flags=re.DOTALL | re.IGNORECASE)
_REASONING_SCRATCHPAD_RE = re.compile(r'<REASONING_SCRATCHPAD>.*?</REASONING_SCRATCHPAD>', flags=re.DOTALL | re.IGNORECASE)
_THOUGHT_RE = re.compile(r'<thought>.*?</thought>', flags=re.DOTALL | re.IGNORECASE)
_RUNTIME_TC_RES = [
    re.compile(rf'<{_tc_name}\b[^>]*>.*?</{_tc_name}>', flags=re.DOTALL | re.IGNORECASE)
    for _tc_name in ("tool_call", "tool_calls", "tool_result", "function_call", "function_calls")
]
_RUNTIME_FUNC_RE = re.compile(
    r'(?:(?<=^)|(?<=[\n\r.!?:]))[ \t]*'
    r'<function\b[^>]*\bname\s*=[^>]*>'
    r'(?:(?:(?!</function>).)*)</function>',
    flags=re.DOTALL | re.IGNORECASE,
)
_RUNTIME_UNTERM_THINK_RE = re.compile(
    r'(?:^|\n)[ \t]*<(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)\b[^>]*>.*$',
    flags=re.DOTALL | re.IGNORECASE,
)
_RUNTIME_STRAY_THINK_RE = re.compile(
    r'</?(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)>\s*',
    flags=re.IGNORECASE,
)
_RUNTIME_STRAY_TC_RE = re.compile(
...
def strip_think_blocks(agent, content: str) -> str:
    ...
    content = _RUNTIME_FUNC_RE.sub('', content)
    # 2. Unterminated reasoning block — open tag at a block boundary
    #    (start of text, or after a newline) with no matching close.
    #    Strip from the tag to end of string.  Fixes #8878 / #9568
    #    (MiniMax M2.7 leaking raw reasoning into assistant content).
    content = _RUNTIME_UNTERM_THINK_RE.sub('', content)
    # 3. Stray orphan open/close tags that slipped through.
    content = _RUNTIME_STRAY_THINK_RE.sub('', content)
    ...
    content = _RUNTIME_STRAY_TC_RE.sub('', content)
    return content
