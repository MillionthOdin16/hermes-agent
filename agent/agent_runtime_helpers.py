...
# ⚡ Bolt: Consolidated multiple sequential re.sub passes for reasoning tags into a single pre-compiled regex pass using capturing groups and backreferences.
_REASONING_BLOCK_PATTERN = re.compile(
    rf"<({'|'.join(_REASONING_TAG_NAMES)})>.*?</\1>",
    re.DOTALL | re.IGNORECASE
)

_TOOL_CALL_BLOCK_PATTERN = re.compile(
    rf"<({'|'.join(_TOOL_CALL_TAG_NAMES)})\b[^>]*>.*?</\1>",
    re.DOTALL | re.IGNORECASE
)
...
def strip_think_blocks(agent, content: str) -> str:
    ...
    content = _REASONING_BLOCK_PATTERN.sub('', content)
    # 1b. Tool-call XML blocks (openclaw/openclaw#67318). Handle the
    #     generic tag names first — they have no attribute gating since
    #     a literal <tool_call> in prose is already vanishingly rare.
    content = _TOOL_CALL_BLOCK_PATTERN.sub('', content)
