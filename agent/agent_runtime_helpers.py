...
# Pre-compile static regular expressions to prevent redundant compilation overhead
_THINK_BLOCK_RE = re.compile(r'<(think|thinking|reasoning|REASONING_SCRATCHPAD|thought)>.*?</\1>', flags=re.DOTALL | re.IGNORECASE)
_TOOL_CALL_BLOCK_RE = re.compile(r'<(tool_call|tool_calls|tool_result|function_call|function_calls)\b[^>]*>.*?</\1>', flags=re.DOTALL | re.IGNORECASE)
_FUNCTION_BLOCK_RE = re.compile(r'(?:(?<=^)|(?<=[\n\r.!?:]))[ \t]*<function\b[^>]*\bname\s*=[^>]*>(?:(?:(?!</function>).)*)</function>', flags=re.DOTALL | re.IGNORECASE)
_UNTERMINATED_REASONING_RE = re.compile(r'(?:^|\n)[ \t]*<(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)\b[^>]*>.*$', flags=re.DOTALL | re.IGNORECASE)
_STRAY_REASONING_TAG_RE = re.compile(r'</?(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)>\s*', flags=re.IGNORECASE)
_STRAY_TOOL_CALL_CLOSER_RE = re.compile(r'</(?:tool_call|tool_calls|tool_result|function_call|function_calls|function)>\s*', flags=re.IGNORECASE)
...
def strip_think_blocks(agent, content: str) -> str:
    ...
    content = _STRAY_REASONING_TAG_RE.sub('', content)
    ...
    content = _STRAY_TOOL_CALL_CLOSER_RE.sub('', content)
    return content
