...
_CLOSED_THINK_TAGS_RE = re.compile(
    r'<(think|thinking|reasoning|REASONING_SCRATCHPAD|thought)>.*?</\1>',
    flags=re.DOTALL | re.IGNORECASE
)

_TOOL_CALL_BLOCKS_RE = re.compile(
...
_STRAY_THINK_TAGS_RE = re.compile(
    r'</?(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)>\s*',
    flags=re.IGNORECASE
)

_STRAY_TOOL_TAGS_RE = re.compile(
...
def strip_think_blocks(agent, content: str) -> str:
    ...
    content = _CLOSED_THINK_TAGS_RE.sub('', content)
    # 1b. Tool-call XML blocks (openclaw/openclaw#67318). Handle the
    #     generic tag names first — they have no attribute gating since
    #     a literal <tool_call> in prose is already vanishingly rare.
    content = _TOOL_CALL_BLOCKS_RE.sub('', content)
    ...
    content = _UNTERMINATED_THINK_RE.sub('', content)
    # 3. Stray orphan open/close tags that slipped through.
    content = _STRAY_THINK_TAGS_RE.sub('', content)
    ...
    content = _STRAY_TOOL_TAGS_RE.sub('', content)
    return content
