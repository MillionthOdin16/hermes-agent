...
_REASONING_CLOSED_RES = [
    re.compile(rf"<{tag}>.*?</{tag}>\s*", flags=re.DOTALL | re.IGNORECASE) for tag in _REASONING_TAGS
]
_REASONING_UNTERM_RES = [
    re.compile(rf"<{tag}>.*$", flags=re.DOTALL | re.IGNORECASE) for tag in _REASONING_TAGS
]
_REASONING_ORPHAN_RES = [
    re.compile(rf"</{tag}>\s*", flags=re.IGNORECASE) for tag in _REASONING_TAGS
]
...
def _strip_reasoning_tags(text: str) -> str:
    ...
    cleaned = text
    for r in _REASONING_CLOSED_RES:
        cleaned = r.sub("", cleaned)
    for r in _REASONING_UNTERM_RES:
        cleaned = r.sub("", cleaned)
    for r in _REASONING_ORPHAN_RES:
        cleaned = r.sub("", cleaned)
    for r in _TC_RES:
        cleaned = r.sub("", cleaned)
    cleaned = _FUNC_RE.sub("", cleaned)
    cleaned = _STRAY_TC_RE.sub("", cleaned)
