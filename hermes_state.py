...
def format_session_db_unavailable(prefix: str = "Session database not available") -> str:
    ...
    if any(marker in lowered_cause for marker in _WAL_INCOMPAT_MARKERS):
...
def is_malformed_db_error(exc: BaseException) -> bool:
    ...
    if not isinstance(exc, sqlite3.DatabaseError):
        return False
    lowered = str(exc).lower()
    return any(marker in lowered for marker in _MALFORMED_DB_MARKERS)
...
def is_malformed_schema_error(exc: BaseException) -> bool:
    ...
    if not isinstance(exc, sqlite3.DatabaseError):
        return False
    lowered = str(exc).lower()
    return any(marker in lowered for marker in _MALFORMED_SCHEMA_MARKERS)
