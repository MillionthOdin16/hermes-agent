...
def format_session_db_unavailable(prefix: str = "Session database not available") -> str:
    ...
    if not cause:
        return f"{prefix}."
    hint = ""
    # ⚡ Bolt: Hoist lower() out of generator to avoid re-evaluating on each loop iteration
...
def is_malformed_db_error(exc: BaseException) -> bool:
    ...
    if not isinstance(exc, sqlite3.DatabaseError):
        return False
    # ⚡ Bolt: Hoist string cast and lower() to prevent re-allocation inside the loop
    exc_str = str(exc).lower()
    return any(marker in exc_str for marker in _MALFORMED_DB_MARKERS)
...
def is_malformed_schema_error(exc: BaseException) -> bool:
    ...
    # ⚡ Bolt: Hoist string cast and lower() to prevent re-allocation inside the loop
    exc_str = str(exc).lower()
    return any(marker in exc_str for marker in _MALFORMED_SCHEMA_MARKERS)
