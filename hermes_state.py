...
def format_session_db_unavailable(prefix: str = "Session database not available") -> str:
    ...
    if not cause:
        return f"{prefix}."
    hint = ""
    # ⚡ Bolt: Hoisting string evaluation to prevent redundant allocations
...
def is_malformed_db_error(exc: BaseException) -> bool:
    ...
    return any(marker in exc_lower for marker in _MALFORMED_DB_MARKERS)
...
def is_malformed_schema_error(exc: BaseException) -> bool:
    ...
    # ⚡ Bolt: Hoisting string evaluation to prevent redundant allocations
    exc_lower = str(exc).lower()
    return any(marker in exc_lower for marker in _MALFORMED_SCHEMA_MARKERS)
