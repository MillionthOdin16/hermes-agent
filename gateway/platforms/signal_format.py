...
_NORM_BULLETS_SPLIT_RE = re.compile(r"(```.*?```)", flags=re.DOTALL)
_NORM_BULLETS_SUB_RE = re.compile(r"(?m)^([ \t]{0,3})[-*+]\s+")
_NEWLINE_3_RE = re.compile(r"\n{3,}")
...
def markdown_to_signal(text: str) -> tuple[str, list[str]]:
    ...
    def _normalize_bullet_markers(source: str) -> str:
        ...
        """
        parts = _NORM_BULLETS_SPLIT_RE.split(source)
        for idx, part in enumerate(parts):
            if idx % 2 == 1:
                continue
            parts[idx] = _NORM_BULLETS_SUB_RE.sub(r"\1• ", part)
        return "".join(parts)

    text = _NEWLINE_3_RE.sub("\n\n", text)
    text = text.strip()
